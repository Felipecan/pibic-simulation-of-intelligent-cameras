...
void VirtualBusFederate::loop (int iteracoes)
{
	...
	{	
		...
		//advanceTime(1.0);
		cout << ">>>>>>>>>>>> Terminal Servidor <<<<<<<<<<<<\n" << endl;
		//cout << "Digite o numero correspondente a opção, caso não queira fazer nada, não digite" << endl;
		//cout << "[1] Consultar face na camera" << endl;
		//cout << "[2] Parar consulta na camera" << endl;
		//cout << "---------> ";

		//tratar entrada ainda....
		//cin >> op;

		cout << "Consultando camera " << cam << endl;

		switch(op)
		{

			case 1:
				//cout << "Digite o numero da camera para consulta-la ";
				//cin >> cam;
				consultTerminalCam(cam);
				break;

			case 2:
				//stopConsultTerminalCam(&numberCam, &server, &size, &data1, &data2, &data3, &data4)
				break;
 ...
 */
void VirtualBusFederate::updateAttributeValues(unsigned src, unsigned addr, unsigned size, unsigned data1, unsigned data2, unsigned data3, unsigned data4)
{
	///////////////////////////////////////////////
...
void VirtualBusFederate::consultTerminalCam(unsigned c)
{

	unsigned numberCam = c; //número da câmera para consulta
	unsigned server = 0; //determina que a mensagem está sendo enviada do servidor
	unsigned data2 = 1; //
	...
	writeData(numberCam, server, time, time, data2, data3, data4);
	...
	while(loopDetect)
	{
		...
		if((readData(numberCam, server, time, data1, data2, data3, data4)) && (server != 0))
		{
